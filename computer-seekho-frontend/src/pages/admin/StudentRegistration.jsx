...
const StudentRegistration = () => {
    ...
    const handlePaymentAndRegistration = async () => {
        ...
        try {
            ...
            // 2. Process Payment
            // Assuming Payment Type ID 3 is "Online Transfer" or similar. Defaulting to 1 (Cash) if unsure, but user asked for functionality.
            // Let's use 3 for Online/Card simulation.
            const paymentPayload = {
                student: { studentId: newStudent.studentId },
                amount: parseFloat(formData.paymentDue),
            ...
            // Success
            setStatus({ type: 'success', msg: 'Student Registered and Payment of ₹' + formData.paymentDue + ' Processed Successfully!' });
            setEnquiry(null);
            setSearchPhone('');
            ...
            <Modal show={showPaymentModal} onHide={() => setShowPaymentModal(false)} backdrop="static" centered>
                ...
                <Modal.Body className="p-4">
                    <div className="text-center mb-4">
                        ...
                        <h2 className="text-primary">₹{formData.paymentDue?.toLocaleString()}</h2>
                    </div>

                    <Form>
                        <Form.Group className="mb-3">
                            <Form.Label>Card Number</Form.Label>
                            <Form.Control type="text" placeholder="XXXX-XXXX-XXXX-XXXX" defaultValue="4242-4242-4242-4242" />
                        </Form.Group>
                        <Row>
                            <Col>
                                <Form.Group className="mb-3">
                                    <Form.Label>Expiry</Form.Label>
                                    <Form.Control type="text" placeholder="MM/YY" defaultValue="12/28" />
                                </Form.Group>
                            </Col>
                            <Col>
                                <Form.Group className="mb-3">
                                    <Form.Label>CVV</Form.Label>
                                    <Form.Control type="text" placeholder="123" defaultValue="123" />
                                </Form.Group>
                            </Col>
                        </Row>
                        <Form.Group className="mb-3">
                            <Form.Label>Card Holder Name</Form.Label>
                            <Form.Control type="text" defaultValue={formData.studentName} />
                        </Form.Group>
                    </Form>

                    {isProcessing && <div className="text-center my-3"><Alert variant="info">Processing Transaction...</Alert></div>}
                ...
                <Modal.Footer>
                    ...
                    <Button variant="success" size="lg" onClick={handlePaymentAndRegistration} disabled={isProcessing}>
                        {isProcessing ? 'Processing...' : `Pay ₹${formData.paymentDue}`}
                    </Button>
                </Modal.Footer>
            </Modal>
        </Container>
